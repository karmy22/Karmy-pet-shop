...
function buildUnsplashUrl(path, params = {}) {
...
async function unsplashRequest(path, params = {}) {
  if (!ACCESS_KEY) {
    throw new Error('REACT_APP_UNSPLASH_ACCESS_KEY is not set. Add it to your .env file.');
  }
  const response = await fetch(buildUnsplashUrl(path, params));
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data.errors?.[0] || `Unsplash request failed: ${response.status}`);
  }
...
export async function getRandomUnsplashPhoto(query, options = {}) {
  ...
  const data = await unsplashRequest('/photos/random', {
    query,
    count,
    ...(orientation ? { orientation } : {}),
  });
...
export async function getUnsplashPhoto(photoId) {
  const data = await unsplashRequest(`/photos/${photoId}`);
